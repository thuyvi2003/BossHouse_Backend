...
exports.removePromotion = async (promotionId) => {
  const promotionDeleted = await Promotion.findByIdAndUpdate(
    promotionId,
    { is_hidden: true },
    { new: true }
  );
...
exports.getAvailablePromotion = async (userId) => {
  ...
  try {
    ...
    promotions = promotions.filter((p) => {
      ...
      const hasUses = p.max_uses == null || p.used_count < p.max_uses;
  ...
  console.log("Promotions available:", promotions);
  //Filter promotion user claimed
  const claimed = await UserPromotion.find({ user_id: userId }).select(
    "promotion_id"
  );
  console.log("Claimed promotions:", claimed);
  const claimedIds = claimed.map((c) => {
    return c.promotion_id.toString();
  });
...
exports.applyPromotion = async (userId, promotionId) => {
  ...
  if (promo.min_order_value && total < promo.min_order_value) {
    throw new Error(
      `Cart total must be at least ${promo.min_order_value}đ to apply this promotion`
    );
  }
...
exports.claimPromotion = async (userId, promotionId) => {
  ...
  if (existed)
    throw new Error(
      "You already claimed this promotion please select other promotion"
    );
